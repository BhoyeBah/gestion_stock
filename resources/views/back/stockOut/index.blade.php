...
<style>
    ...
    .page-header h1 { color: #fff; font-weight: 700; margin: 0; font-size: 1.75rem; }
    ...
    .stock-table tbody td { padding: 1rem 0.75rem; vertical-align: middle; }
</style>
                ...
                <table class="table stock-table">
                    ...
                    <tbody>
                        @foreach($stockOuts as $stock)
                            <tr>
                                <td>{{ $loop->iteration + ($stockOuts->currentPage() - 1) * $stockOuts->perPage() }}</td>
                                <td>{{ $stock->batch->product->name ?? '-' }} / {{ $stock->batch->name ?? '-' }}</td>
                                <td>{{ $stock->quantity }}</td>
                                <td>{{ $stock->reason ?? '-' }}</td>
                                <td>{{ $stock->created_at->format('d/m/Y H:i') }}</td>
                                <td class="text-center">
                                    <form action="{{ route('stockout.destroy', $stock->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Confirmer la suppression ?')">
                                        @csrf
                                        @method('DELETE')
            ...
            <form action="{{ route('stockout.store') }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label for="batch_id">Produit / Lot</label>
                        <select name="batch_id" id="batch_id" class="form-control" required>
                            <option value="">Sélectionnez un lot</option>
                            @foreach(\App\Models\Batch::with('product')->get() as $batch)
                                <option value="{{ $batch->id }}">{{ $batch->product->name ?? '-' }} / {{ $batch->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
