        ...
        <div class="card-body">
                            ...
                            @foreach ($contacts as $contact)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <!-- Activer / Désactiver -->
                                        <form action="{{ route("$type.toggle", $contact->id) }}" method="POST"
                                            class="d-inline"
                                            onsubmit="return confirm('Voulez-vous {{ $contact->is_active ? 'désactiver' : 'activer' }} ce {{ $type === 'client' ? 'client' : 'fournisseur' }} ?')">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit"
                                                class="btn btn-sm {{ $contact->is_active ? 'btn-success' : 'btn-danger' }}">
                                                <i class="fas fa-toggle-{{ $contact->is_active ? 'off' : 'on' }}"></i>
                                            </button>
                                        </form>

                                        <a href="{{ route("$type.show", $contact->id) }}" class="btn btn-sm btn-info"
                                            title="Voir">
                                            <i class="fas fa-eye"></i>
                                        </a>

                                        <!-- Modifier -->
                                        <a href="{{ route("$type.edit", $contact->id) }}" class="btn btn-sm btn-warning"
                                            title="Modifier">
                                        ...
                                        <form action="{{ route("$type.destroy", $contact->id) }}" method="POST"
                                            class="d-inline"
                                            onsubmit="return confirm('Confirmer la suppression de ce {{ $type === 'client' ? 'client' : 'fournisseur' }} ?')">
                                            @csrf
                                            @method('DELETE')
            ...
            @else
                <div class="alert alert-info text-center">
                    <i class="fas fa-info-circle"></i> Aucun {{ $type === 'clients' ? 'client' : 'fournisseur' }}
                    disponible pour le moment.
                    <br>
                    <small>Créez-en un en cliquant sur le bouton « Nouveau
                        {{ $type === 'clients' ? 'client' : 'fournisseur' }} » ci-dessus.</small>
                </div>
            @endif
        </div>
    </div>
